...
test('ViewRestaurantReviews', async ({ page }) => {
  ...
  await page.waitForSelector('text=/Based on \\d+ review/', { timeout: 50000 });
  ...
  await expect(reviewRatings).toBeVisible({ timeout: 50000 });
});
...
test('ViewRestaurantWithNoReviews', async ({ page }) => {
  ...
  await expect(page.getByText('No reviews yet')).toBeVisible({ timeout: 50000 });
});

...
export default function RegisterRestaurantForm() {
  ...
  const validatePostalCode = (postal: string): boolean =>
    /^[0-9]+$/.test(postal) && postal.trim().length > 0;
  const validateProvince = (): boolean =>
    selectedProvince !== null && selectedProvince.trim().length > 0;
  const validateAvailableTime = (): boolean => {
    if (!availableTimeValue || availableTimeValue.length === 0) return false;

    return availableTimeValue.some((item) => item.openTime && item.closeTime);
  };
  ...
  const handlePriceRangeChange = (text: string) =>
    setPriceRange(text.replace(/[^0-9]/g, ''));
  const handleReservationFeeChange = (text: string) =>
    setReservationFee(text.replace(/[^0-9]/g, ''));
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Update restaurant info with image URLs
      const payloadWithImages = {
        ...payload,
        images: uploadedUrls,
        id: restaurantId,
      };
      await updateRestaurantInfo(payloadWithImages);
    ...
    <View className="h-full w-3/5 bg-white p-16 pr-32 overflow-y-auto">
      <View className="bg-[#FEF9F3] rounded-2xl border border-[#FAE8D1] shadow p-16 mb-16">
        ...
        <View className="flex-row flex-wrap">
          {['House No', 'Village', 'Building', 'Road', 'Soi'].map(
            (label, idx) => {
        ...
        <View className="mb-4">
          <Text
            className={`text-left text-base mb-1 ${hasAttemptedSubmit && !validateAvailableTime() ? 'text-red-500' : 'text-black'}`}
          >
            Available Time{' '}
          ...
          {showAvailableTimeDropdown && (
            <AvailableTimeDropdown
              value={availableTimeValue.map(
                ({ restaurantId, ...rest }) => rest,
              )} // remove restaurantId for dropdown
        ...
        <View className="mb-4">
          <Text
            className={`text-left text-base pb-1 ${hasAttemptedSubmit && !validatePriceRange() ? 'text-red-500' : 'text-black'}`}
          >
        ...
        <View className="mb-4">
          <Text
            className={`text-left text-base pb-1 ${hasAttemptedSubmit && !validateReservationFee() ? 'text-red-500' : 'text-black'}`}
          >
            Reservation Fee{' '}
            {hasAttemptedSubmit && !validateReservationFee() && '*'}
          </Text>
          <View className="flex-row">
            <TextInput
